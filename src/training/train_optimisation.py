...
def objective(trial):
    ...
    INPUT_DIM = 37 
    BATCH_SIZE = 256 # Pourquoi un batch size plus grand que 128 ? Detailler dans le rapport final (avantages d'un batch size plus grand pour la stabilité de l'entraînement et les performances sur ce dataset spécifique)
    EPOCHS = 100  # Pourquoi 100 époques ? Detailler dans le rapport final (justification basée sur les courbes d'apprentissage observées lors des expérimentations préliminaires)
    ...
    criterion = FocalLoss(alpha=1, gamma=2) # Détailler le focal loss dans le rapport final (pourquoi passer du BCE classique au Focal Loss ?)
    optimizer = optim.AdamW(model.parameters(), lr=learning_rate, weight_decay=weight_decay) # Detailler le choix de l'optimiseur et du weight decay dans le rapport final (pourquoi passer d'un optimiseur classique à AdamW ? Pourquoi introduire un weight decay ?)
